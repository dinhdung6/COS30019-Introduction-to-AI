...
from heuristics import manhattan_journey as manhattan_distance, euclidean_journey as euclidean_distance
from gui import run_gui
from search_utilities import ExpeditionGrid
from direction import find_direction

def main(file_path, search_method):
    ...
    def execute_search():
        if search_method == "DFS":
            return depth_first_search(map_grid, start, goals)
        elif search_method == "BFS":
            return breadth_first_search(map_grid, start, goals)
        elif search_method == "GBFS":
            return greedy_best_first_search(map_grid, start, goals, manhattan_distance)
        elif search_method == "AS":
            return a_star_search(map_grid, start, goals, manhattan_distance)
        elif search_method == "CUS1":
            return cus1(map_grid, start, goals)
        elif search_method == "CUS2":
            return cus2(map_grid, start, goals, custom_heuristic)
        else:
    ...
    if path:
        ...
        print([find_direction(path[i], path[i+1]) for i in range(len(path)-1)])
    else:
        print(f"{file_path} {search_method}")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python search.py <file_path> <search_method>")
        sys.exit(1)

    file_path = sys.argv[1]
    search_method = sys.argv[2].upper()
    main(file_path, search_method)
